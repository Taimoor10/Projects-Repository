...
currentnodeurl = '192.168.128.89:8003'
...
def writetofile():
    ret = []
    try:
        blockchain_file = Path("/Users/muhammad.taimoor/PycharmProjects/Blockchain/3rdNode.json")
        if not blockchain_file.exists():
            with open('3rdNode.json', mode='w', encoding='utf-8') as f:
                json.dump([], f)
        else:
            with open('3rdNode.json', mode='w', encoding='utf-8') as feedsjson:
                for block in blockchain:
                    ret.append({
                        "index": block.index,
...
def getdata():
    ...
    for i in range(len(blockchain)):
        if i == 0:
            ...
            calculatedhash = hashblock(currentblock.index, currentblock.timestamp, currentblock.data,
...
def getpatientname():
    if request.method == 'GET':
        ...
        ret = []
        jdata = json.load(open("/Users/muhammad.taimoor/PycharmProjects/Blockchain/3rdNode.json"))
        for data in jdata:
            if data['index'] == 0:
                print("Genesis Block")
            else:
...
def registernodeandbroadcast():
    global blockchain
    ret = []

    response = requests.get("http://192.168.128.89:8002/getblockchain").json()
    peerip = request.environ['REMOTE_ADDR']
...
@app.route('/registernode', methods=['GET', 'POST'])
def registernode():

    try:
        response = request.data.decode('utf-8').replace('"', "")
        peerip = request.environ['REMOTE_ADDR']
        print("Received Message from", peerip)
        print("Node Address:", response)

        print(networknodes)
        if response not in networknodes:
...
if __name__ == '__main__':
    ...
    if blockchain_file.exists():
        if os.stat('3rdNode.json').st_size == 0:
            blockchain.append(creategenesisblock())
            writetofile()
        else:
            with open('3rdNode.json') as f:
                data = json.loads(f.read())
                for b in data:
                    blockchain.append(Blockchain(b['index'], b['timestamp'], b['data'], b['previoushash'], b['hash']))
    socketio.run(app, '192.168.128.89', 8003)